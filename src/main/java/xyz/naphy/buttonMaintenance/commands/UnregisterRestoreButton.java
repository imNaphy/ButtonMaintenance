...
public class UnregisterRestoreButton implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        Location blockLoc = ButtonMaintenance.restoreButtons.get(args[0]);
        blockLoc.getBlock().setType(Material.AIR);
        try {
            ButtonMaintenance.buttonsFile.set("Restore buttons." + args[0], null);
            ButtonMaintenance.buttonsFile.save(new File(ButtonMaintenance.plugin.getDataFolder(), "buttons.yml"));
            ButtonMaintenance.restoreButtons.remove(args[0]);
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7[&bButtonMaintenance&7] &7The restore button with the ID &b" + args[0] + " &7has been unregistered!"));
        } catch (IOException e) {
